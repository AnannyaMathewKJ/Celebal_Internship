...
HF_API_KEY = sanitize_env_value(os.getenv("HF_API_KEY") or os.getenv("HF_TOKEN") or "")
HF_MODEL = sanitize_env_value(os.getenv("HF_MODEL") or "openai/gpt-oss-120b:cerebras")
PORT = 3000
...
# -------------------------------------------------------------
# HUGGING FACE INFERENCE PROVIDERS API INTEGRATION
# -------------------------------------------------------------
def call_huggingface(prompt: str) -> str:
    api_key = HF_API_KEY
    if not api_key:
        raise ValueError("HF_API_KEY is missing. Add your Hugging Face token to .env and restart the server.")
    ...
    try:
        ...
        if error.code == 401:
            raise ValueError(
                "Hugging Face rejected HF_API_KEY. Replace it with a valid Hugging Face access token, "
                "then restart the server."
            ) from error
        if error.code == 403:
            raise ValueError(
                "Your Hugging Face token is valid but lacks Inference Providers permission. Create a new token "
                "with that permission, replace HF_API_KEY in .env, then restart the server."
            ) from error
        raise RuntimeError(f"Hugging Face API error {error.code}: {details}") from error
    except urllib.error.URLError as error:
        raise RuntimeError(
            "Unable to connect to the Hugging Face API. Allow this server to make outbound HTTPS "
            "connections, then restart it and try again."
        ) from error
...
def run_python_code(code: str, dataset_path: str):
    ...
    # Run sandbox process
    try:
        res = subprocess.run([sys.executable, "sandbox.py"], capture_output=True, text=True, timeout=10)
        stdout = res.stdout
        stderr = res.stderr
...
class MyHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path == "/analyze":
            ...
            # Construct base Hugging Face prompt
...
```"""
                    ...
                    # Call Hugging Face
                    model_text = call_huggingface(prompt)
                    
                    # Extract python block
                    import re
                    code_match = re.search(r'```python([\s\S]*?)```', model_text)
                    code_to_run = code_match.group(1).strip() if code_match else model_text.strip()
